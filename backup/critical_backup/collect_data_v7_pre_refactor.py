...
#         'select_smaller_angle_mug': False,  # 可选，不设置则使用全局默认值
#         'tb3_x_random_enabled': True,  # 可选，不设置则使用全局默认值
#         'tray_init_on_tb3_enabled': False,  # 可选，不设置则使用全局默认值
#     },
#     {
#         'name': 'stage_2',
#         'target_episodes': 200,
...
# --- 🧭 指令组配置（左右方向键切换组）---
# 每个模式可配置多个“指令组”，每组可放多条文本指令。
# 运行时会在当前组内随机抽取（尽量避免与上一条重复）。
INSTRUCTION_GROUPS = {
    'arm': [
        {
            'name': 'arm_default',
...
# 托盘模式与默认模式对应的 ARM 指令组名（供全自动按配置强制切换）
...
AUTO_RED_MUG_EXPECTED_Z = TABLE_Z_HEIGHT  # 🔥 红色杯子正常 z 坐标（与环境初始化高度联动）
...
def _validate_instruction_groups():
    ...
    # 全自动模式依赖这两个 ARM 组名做任务语义切换
    arm_group_names = {g.get('name') for g in INSTRUCTION_GROUPS.get('arm', [])}
    for required_name in (AUTO_ARM_GROUP_DEFAULT_NAME, AUTO_ARM_GROUP_TRAY_NAME):
...
def _get_group_info(mode, group_indices):
    """返回当前模式的组信息：(idx, total, group_name, instructions)。"""
...
def _find_group_index_by_name(mode, group_name):
    """按组名查找组索引，找不到返回 None。"""
    groups = INSTRUCTION_GROUPS.get(mode, [])
...
def _route_arm_group_for_auto(group_indices, tray_init_on_tb3_enabled):
    """
    根据托盘开关路由 ARM 指令组：
    - True  -> AUTO_ARM_GROUP_TRAY_NAME
    - False -> AUTO_ARM_GROUP_DEFAULT_NAME
    """
    target_group_name = AUTO_ARM_GROUP_TRAY_NAME if tray_init_on_tb3_enabled else AUTO_ARM_GROUP_DEFAULT_NAME
    target_idx = _find_group_index_by_name('arm', target_group_name)
...
def _apply_instruction_from_group(
    PnPEnv,
    mode,
    group_indices,
    ...
    picked_instruction = _pick_instruction_from_active_group(
        mode,
        group_indices,
        last_instruction_by_mode,
    )
    ...
    print(
        ...
        f"({idx + 1}/{total}) | Instruction: {PnPEnv.instruction}"
    )
...
def main():
    ...
    print(
        f"🚗 TB3 X Randomization: {'Uniform' if TB3_X_RANDOM_ENABLED else 'Fixed'} "
        f"(range=[{TB3_X_MIN:.3f}, {TB3_X_MAX:.3f}])"
    )
    ...
    # tray_init_on_tb3_enabled 在全自动中用于“路由指令组”，
    # 初始化是否上托盘由指令文本自然决定，这里不做强制覆盖。
    PnPEnv.force_tray_init_enabled = False
    ...
    auto_instruction_prepared = False       # 🔥 当前轮次指令是否已在 reset 前准备好
    ...
    _apply_instruction_from_group(
        ...
        last_instruction_by_mode,
        log_prefix="[INIT]",
        reinitialize_arm=(current_mode == 'arm'),
    )
            ...
            if PnPEnv.env.loop_every(HZ=FPS):
                ...
                if auto_state != AUTO_STATE_IDLE and current_mode == 'arm':
                    ...
                    if auto_state == AUTO_STATE_RESETTING:
                        ...
                        if auto_use_multi_config and auto_current_config is not None:
                            ...
                            # 🔥 TB3 X 随机化参数：支持每个配置覆盖
                            config_tb3_x_random_enabled = auto_current_config.get('tb3_x_random_enabled', TB3_X_RANDOM_ENABLED)
                            config_tb3_x_min = auto_current_config.get('tb3_x_min', TB3_X_MIN)
                            config_tb3_x_max = auto_current_config.get('tb3_x_max', TB3_X_MAX)
                            config_tray_init_on_tb3 = auto_current_config.get('tray_init_on_tb3_enabled', TRAY_INIT_ON_TB3_ENABLED)
                            # 更新环境的随机初始化参数
                            ...
                            # 🔥 更新环境的 TB3 X 随机化参数
                            PnPEnv.tb3_x_random_enabled = config_tb3_x_random_enabled
                            PnPEnv.tb3_x_min = config_tb3_x_min
                            PnPEnv.tb3_x_max = config_tb3_x_max
                            # tray_init_on_tb3_enabled 现在只负责“切换指令组语义”，
                            # 初始化是否上托盘由指令内容（tray->table）自然触发。
                            PnPEnv.force_tray_init_enabled = False
                        else:
                            # 单配置模式：使用全局默认参数
                            ...
                            config_tb3_x_random_enabled = TB3_X_RANDOM_ENABLED
                            config_tb3_x_min = TB3_X_MIN
                            config_tb3_x_max = TB3_X_MAX
                        ...
                        # 🔥 先准备当轮指令（用于初始化与目标颜色对齐），再 reset 保留该指令
                        _apply_instruction_from_group(
                            PnPEnv,
                            'arm',
                            instruction_group_indices,
                        ...
                        auto_instruction_prepared = True
                    ...
                    # ----- STATE: START_EXPERT -----
                        ...
                        if getattr(PnPEnv, 'moving_to_random', False):
                            ...
                            if not auto_instruction_prepared:
                                _apply_instruction_from_group(
                                    PnPEnv,
                                    'arm',
                                    instruction_group_indices,
                            ...
                            auto_instruction_prepared = False
                ...
                if PnPEnv.env.is_key_pressed_once(glfw.KEY_C):
                    if auto_state != AUTO_STATE_IDLE:
                        ...
                        if hasattr(PnPEnv, 'base_auto_active'):
                            PnPEnv.base_auto_active = False
                        if hasattr(PnPEnv, 'base_auto_stage'):
                            PnPEnv.base_auto_stage = 'idle'
                        if hasattr(PnPEnv, 'base_auto_wait_counter'):
                            PnPEnv.base_auto_wait_counter = 0
                        if hasattr(PnPEnv, 'base_auto_stage_steps'):
                            PnPEnv.base_auto_stage_steps = 0
                        if hasattr(PnPEnv, 'base_auto_record_stop_requested'):
                            PnPEnv.base_auto_record_stop_requested = False
                ...
                # [←]/[→] 切换“当前模式”的指令组（全自动模式下禁用）
                left_pressed = PnPEnv.env.is_key_pressed_once(glfw.KEY_LEFT)
                right_pressed = PnPEnv.env.is_key_pressed_once(glfw.KEY_RIGHT)
                if left_pressed or right_pressed:
                    if auto_state != AUTO_STATE_IDLE:
                        ...
                        groups = INSTRUCTION_GROUPS[current_mode]
                        if len(groups) <= 1:
                            idx, total, group_name, _ = _get_group_info(current_mode, instruction_group_indices)
                            print(f"\nℹ️ [{current_mode.upper()}] Only one instruction group: {group_name} ({idx + 1}/{total})")
                        else:
                            if right_pressed:
                                instruction_group_indices[current_mode] = (instruction_group_indices[current_mode] + 1) % len(groups)
                            else:
                                instruction_group_indices[current_mode] = (instruction_group_indices[current_mode] - 1) % len(groups)
                            _apply_instruction_from_group(
                                PnPEnv,
                ...
                # 机器人控制
                # 同步 Base 自动停车是否处于录制中（供 H 流程决定是否等待 60 帧）
                if hasattr(PnPEnv, 'base_auto_recording_active'):
                    PnPEnv.base_auto_recording_active = bool(current_mode == 'base' and is_recording)
                ...
                if reset:
                    if auto_state != AUTO_STATE_IDLE:
                        ...
                        if current_mode == 'arm':
                            PnPEnv._waiting_for_save = False
                            PnPEnv.expert_auto_save = False  # 🔥 重置自动保存标志
                        PnPEnv.reset(mode=current_mode, preserve_instruction=True)
                        # 🔥 先清空队列（防止新数据继续进入）
                        if worker.clear_queue():
                            # 🔥 等待正在处理的帧完成并写入缓冲区（确保所有数据都被捕获）
                            worker.wait_queue_empty()
                ...
                # 🚗 Base 模式：L 键自动停车流程结束后，按当前录制状态决定是否自动停录保存
                if current_mode == 'base' and getattr(PnPEnv, 'base_auto_record_stop_requested', False):
                    h_autosave_just_finished = False
                    if worker.saving_in_progress:
                        # 正在保存时先不消费该信号，下一帧再处理，避免并发保存冲突
                        ...
                        if is_recording:
                            ...
                            try:
                                ...
                                episode_ids[current_mode] += 1
                                # H 键自动停车在保存后自动重置环境，
                                # 让红/蓝杯子重新随机到新的初始位置，同时机械臂回到固定初始位。
                                try:
                                    PnPEnv.reset(mode=current_mode, force_fixed_arm_init=True)
                        ...
                        # 只消费一次完成信号
                        PnPEnv.base_auto_record_stop_requested = False
                        # 强保证：H 自动保存后，本帧后续不再执行任何数据路径，
                        # 避免 reset 后的画面被混入刚保存完的上一条数据。
