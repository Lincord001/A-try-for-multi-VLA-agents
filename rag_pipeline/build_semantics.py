...
def parse_args() -> SemanticArgs:
    ...
    parser.add_argument(
        "--output_json",
        type=str,
        default="topology_output/semantic_topological_map_multimodal.json",
        help="输出语义拓扑图 JSON 路径",
    )
    parser.add_argument(
        "--caption_output_json",
        type=str,
        default="topology_output/semantic_topological_map_multimodal_captions_only.json",
...
class SemanticAnnotator:
    ...
    def __init__(self, args: SemanticArgs):
        ...
        LOGGER.info("使用 DashScope 多模态向量模型: tongyi-embedding-vision-plus")
    ...
    @staticmethod
    def _collect_embedding_vectors(payload: Any) -> List[List[float]]:
        vectors: List[List[float]] = []
        if isinstance(payload, (list, tuple)):
            if payload and all(isinstance(x, (int, float)) for x in payload):
                vectors.append([float(x) for x in payload])
                return vectors
            for item in payload:
                vectors.extend(SemanticAnnotator._collect_embedding_vectors(item))
            return vectors
        if isinstance(payload, dict):
            for key in ("embedding", "embeddings", "vector", "vectors"):
                if key in payload:
                    vectors.extend(SemanticAnnotator._collect_embedding_vectors(payload[key]))
            for sub in payload.values():
                vectors.extend(SemanticAnnotator._collect_embedding_vectors(sub))
        return vectors

    def _extract_embedding(self, response: Any) -> List[float]:
        if response is None:
            raise RuntimeError("DashScope embedding 返回为空")

        status_code = getattr(response, "status_code", None)
        if status_code is not None and int(status_code) != 200:
            message = getattr(response, "message", None) or str(response)
            raise RuntimeError(f"DashScope embedding 调用失败: status_code={status_code}, message={message}")

        output_obj = getattr(response, "output", None)
        if output_obj is None and isinstance(response, dict):
            output_obj = response.get("output")

        vectors = self._collect_embedding_vectors(output_obj if output_obj is not None else response)
        if not vectors:
            raise RuntimeError(f"无法从 DashScope embedding 响应中提取向量: {response}")
        return vectors[0]
    ...
    def _generate_multimodal_embedding(self, image_path: Path, caption: str) -> List[float]:
        # 每次调用都做异常透传，交给 Tenacity 重试控制。
        try:
            ...
            LOGGER.warning("多模态 embedding 调用异常，将重试: image=%s err=%s", image_path, exc)
            raise
    ...
    def run(self) -> None:
        ...
        for node_id, node_attr in tqdm(nodes_data, desc="SemanticAnnotating", unit="node"):
            ...
            try:
                ...
                embedding = self._generate_multimodal_embedding(panorama_abs, caption)
