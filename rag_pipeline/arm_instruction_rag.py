...
SELECT_PROMPT_TEMPLATE = (
    ...
    "2. If the query explicitly specifies an object attribute such as color, count, or source/destination relation, the selected candidate must preserve that explicit attribute. In particular, if the query says red, you must not select blue; if the query says blue, you must not select red. An explicit color mismatch is a hard mismatch.\n"
    "3. Treat object identity semantically rather than by strict surface wording. Near-synonymous object names or naming-granularity differences may still match when they plausibly refer to the same physical object category in the current tabletop domain, but clearly different object types must not be merged.\n"
    "4. Historical training labels for locations, containers, or supporting surfaces may be imperfect. In this instruction library, plate and tray may be treated as equivalent receiving-surface labels when they refer to the same manipulation role. More generally, when the query and candidate both refer to a nearby receiving surface or tabletop support in the same manipulation context, allow approximate matching instead of requiring exact literal wording.\n"
    "5. If the user's task and a candidate express the same high-level receiving-surface placement or transfer skill, prefer semantic equivalence over strict literal token matching, but only when doing so does not violate rules 2 or 3 and does not reverse the action direction.\n"
    "6. Return NO_MATCH if every same-skill-family candidate conflicts with an explicit attribute in the query, including explicit color, count, source/destination relation, or a clearly different object type.\n"
